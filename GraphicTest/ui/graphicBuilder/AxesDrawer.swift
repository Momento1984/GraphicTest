...
class AxesDrawer {
  ...
  func drawAxesInRect(bounds: CGRect, origin: CGPoint, pointsPerUnit: CGFloat) {
    ...
    path.stroke()
    //drawHashmarksInRect(frame: frame, pointsPerUnit: abs(pointsPerUnit))
    UIGraphicsGetCurrentContext()!.restoreGState()
  }
  
  func drawAxesInCenterRect(frame: CGRect, pointsPerUnit: CGFloat) {
    UIGraphicsGetCurrentContext()!.saveGState()
    color.set()
    let path = UIBezierPath()
    path.move(to: CGPoint(x: frame.minX, y: align(coordinate: frame.center.y)))
    path.addLine(to: CGPoint(x: frame.maxX, y: align(coordinate: frame.center.y)))
    path.move(to: CGPoint(x: align(coordinate: frame.center.x), y: frame.minY))
    path.addLine(to: CGPoint(x: align(coordinate: frame.center.x), y: frame.maxY))
    path.stroke()
    drawHashmarksInCenterRect(frame: frame, pointsPerUnit: abs(pointsPerUnit))
    UIGraphicsGetCurrentContext()!.restoreGState()
  }
  
  // the rest of this class is private
  private func drawHashmarksInCenterRect(frame: CGRect, pointsPerUnit: CGFloat) {
    ...
    var bbox = CGRect(center: frame.center, size: CGSize(width: bboxSize, height: bboxSize))
    ...
    while !bbox.contains(frame) {
      
      let label = formatter.string(from: NSNumber(value: Double(((frame.maxX / 2 - bbox.minX) / pointsPerUnit))))!
      if let leftHashmarkPoint = alignedPoint(x: bbox.minX, y: frame.center.y, insideBounds: frame) {
        drawHashmarkAtLocation(leftHashmarkPoint, .Top("-\(label)"))
      }
      if let rightHashmarkPoint = alignedPoint(x: bbox.maxX, y: frame.center.y, insideBounds: frame) {
        drawHashmarkAtLocation(rightHashmarkPoint, .Top(label))
      }
      if let topHashmarkPoint = alignedPoint(x: frame.center.x, y: bbox.minY, insideBounds: frame) {
        drawHashmarkAtLocation(topHashmarkPoint, .Left(label))
      }
      if let bottomHashmarkPoint = alignedPoint(x: frame.center.x, y: bbox.maxY, insideBounds: frame) {
