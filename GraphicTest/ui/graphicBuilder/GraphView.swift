...
class GraphView: UIView {
  ...
  override func draw(_ rect: CGRect) {
    //bounds: rect, origin: CGPoint(x: 30, y: rect.size.height - 20)
    axesDrawer.drawAxesInCenterRect(frame: rect, pointsPerUnit: 1)
    for serie in series{
      graphDrawer.color = serie.color
      graphDrawer.drawLinesInCenterRect(frame: rect, pointsPerUnit: 1, points: serie.points)
    }
  }
