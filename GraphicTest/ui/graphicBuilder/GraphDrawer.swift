...
class GraphDrawer {
  ...
  func drawLinesInCenterRect(frame: CGRect, pointsPerUnit: CGFloat, points: [CGPoint]) {
    UIGraphicsGetCurrentContext()!.saveGState()
    color.set()
    let path = UIBezierPath()
    path.move(to: CGPoint(x: align(coordinate:points[0].x + frame.center.x), y: align(coordinate: frame.center.y - points[0].y)))
    for point in points{
      path.addLine(to: CGPoint(x: align(coordinate:point.x + frame.center.x), y: align(coordinate: frame.center.y - point.y)))
    }
